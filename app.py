...
st.set_page_config(
    page_title="Relatório Fotográfico", page_icon="📷", layout="wide"
)
...
# Definição dos arquivos de imagem locais
LOGO_PATH = "logo.png"
...
with col_titulo:
  ...
  st.markdown(
      "Gerador automatizado de relatórios técnicos com layout timbrado."
  )
...
with col_c2:
  nome_cliente = st.text_input(
      "Nome / Razão Social",
      placeholder="Ex: SABOR DA TERRA ALIMENTACAO CORPORATIVA",
  )
...
def carregar_fotos(label, max_arquivos=None):
  ...
  if max_arquivos and fotos and len(fotos) > max_arquivos:
    st.error(
        f"⚠️ Limite excedido para {label}. Serão considerados apenas os"
        f" primeiros {max_arquivos} arquivos."
    )
...
class RelatorioPDF(FPDF):
  ...
  def __init__(self, cod_cliente="", nome_cliente=""):
    super().__init__()
    # Remove pontos e força a conversão para CAIXA ALTA
    self.cod_cliente = (
        cod_cliente.replace(".", "").strip().upper() if cod_cliente else ""
    )
    self.nome_cliente = nome_cliente.strip().upper() if nome_cliente else ""

  def header(self):
    # Aplica o timbrado.png como plano de fundo fixo em todas as páginas
    ...
    # Exibe título e dados do cliente centralizados APENAS na primeira página
    if self.page_no() == 1:
      self.set_y(15)
...
def gerar_pdf(equipamentos, dic_fotos, cod_cliente, nome_cliente):
  ...
  if equipamentos:
    ...
    vazao_total_str = (
        f"{total_vazao:.2f}".replace(".", ",").rstrip("0").rstrip(",")
    )
    pdf.ln(2)
  ...
  for categoria, arquivos in dic_fotos.items():
    if arquivos:
      # Nome do item alinhado à esquerda e sem numeração (ex: FACHADA)
      pdf.set_font("Arial", "B", 11)
...
if st.button("📄 Gerar Relatório PDF"):
  ...
  # Nome do arquivo final no formato: fotos_<código do cliente sem pontos>.pdf
  cod_formatado = (
      cod_cliente.replace(".", "").strip().upper() if cod_cliente else ""
  )
  nome_arquivo_pdf = (
      f"fotos_{cod_formatado}.pdf" if cod_formatado else "fotos.pdf"
  )
...
with col_qtd:
  qtd_input = st.number_input(
      "Quantidade", min_value=1, value=1, step=1, key="eq_qtd"
  )
with col_eq:
  nome_eq_input = st.text_input(
      "Equipamento", placeholder="Ex: Forno Industrial", key="eq_nome"
  )
with col_vaz:
  vazao_input = st.text_input(
      "Vazão Unitária (kg/h)", placeholder="Ex: 1 ou 1,6", key="eq_vazao"
  )
  ...
  if st.button("➕ Adicionar", key="btn_add_eq"):
    if nome_eq_input.strip() and vazao_input.strip():
      try:
        ...
        vazao_formatada = (
            f"{vazao_total_item:.2f}".replace(".", ",").rstrip("0").rstrip(",")
        )
        ...
        item_dict = {
            ...
            "texto": (
                f"{qtd:02d} - {nome_eq_input.strip().upper()} -"
                f" {vazao_formatada} kg/h"
            ),
        }
